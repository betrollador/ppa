...
public class WsClient {
    ...
    public List<EntradaProdutoPreAnalise> doBuscaEntradaProdutoPreAnalise(String busca){

        List<EntradaProdutoPreAnalise> resultado = new ArrayList<EntradaProdutoPreAnalise>();
        ObjectMapper mapper = new ObjectMapper();
        String json = "";
        try {

            URL url = new URL("http://localhost:8080/ppa/api/entradaprodutopreanalise/"+busca);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            conn.setRequestProperty("Accept", "application/json");
        ...
        try {
            resultado = Arrays.asList(mapper.readValue(json, EntradaProdutoPreAnalise[].class));
        } catch (JsonProcessingException e) {
            e.printStackTrace();
        }
